#!/usr/bin/env python3
"""Apply wizard_input column to agents table in tenant databases."""
...
WIZARD_INPUT_SQL = """
-- Add wizard_input JSONB column to agents table
ALTER TABLE agents
ADD COLUMN IF NOT EXISTS wizard_input JSONB;

-- Add comment explaining the column
COMMENT ON COLUMN agents.wizard_input IS 'Raw wizard session step_data stored at creation time. Preserves original user intent for personality, abilities, voice settings, etc.';
""".strip()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply wizard_input column to agents table across tenant databases.",
    )
    parser.add_argument(
        "--client",
        action="append",
...
def run_patch(project_ref: str, label: str, token: str, dry_run: bool) -> None:
    if dry_run:
        print(f"DRY RUN: would apply wizard_input to {label}")
        return
    ok, detail = execute_sql(project_ref, token, WIZARD_INPUT_SQL)
    if ok:
        print(f"{format_status(True)} {label}: wizard_input column applied")
    else:
...
def main() -> int:
    ...
    for row in clients:
        ...
        if status not in {"ready", "schema_syncing"}:
            print(f"Skipping client {client_id}: provisioning_status={status}")
            continue

        supabase_url = row.get("supabase_url")
        if not supabase_url:
            print(f"Skipping client {client_id}: missing supabase_url")
            continue
