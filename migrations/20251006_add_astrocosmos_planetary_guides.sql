-- Seed AstroCosmos Planetary Guide abilities (global n8n webhooks)
-- Uses the existing n8n tool template (webhook_url, timeout, include_context, strip_nulls, user_inquiry_field, default_payload, system_prompt_instructions)

-- Sun Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'sun_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Sun Planetary Guide',
  'sun_planetary_guide',
  'AstroCosmos Sun Guide via webhook (vitality, life force, joy).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Moon Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'moon_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Moon Planetary Guide',
  'moon_planetary_guide',
  'AstroCosmos Moon Guide via webhook (nurturing, feeling, comfort).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Mercury Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'mercury_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Mercury Planetary Guide',
  'mercury_planetary_guide',
  'AstroCosmos Mercury Guide via webhook (curiosity, agility, intelligence).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Venus Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'venus_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Venus Planetary Guide',
  'venus_planetary_guide',
  'AstroCosmos Venus Guide via webhook (kindness, empathy, love, beauty).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Mars Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'mars_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Mars Planetary Guide',
  'mars_planetary_guide',
  'AstroCosmos Mars Guide via webhook (assertiveness, calm strength, courage).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Jupiter Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'jupiter_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Jupiter Planetary Guide',
  'jupiter_planetary_guide',
  'AstroCosmos Jupiter Guide via webhook (generosity, collaboration, inspiration).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Saturn Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'saturn_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Saturn Planetary Guide',
  'saturn_planetary_guide',
  'AstroCosmos Saturn Guide via webhook (responsibility, commitment, wisdom).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Uranus Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'uranus_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Uranus Planetary Guide',
  'uranus_planetary_guide',
  'AstroCosmos Uranus Guide via webhook (innovation, uniqueness, invention).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Neptune Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'neptune_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Neptune Planetary Guide',
  'neptune_planetary_guide',
  'AstroCosmos Neptune Guide via webhook (vision, imagination, transcendence).',
  'n8n',
  'global',
  jsonb_build_object(
...
-- Pluto Planetary Guide
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'pluto_planetary_guide'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Pluto Planetary Guide',
  'pluto_planetary_guide',
  'AstroCosmos Pluto Guide via webhook (transformation, fearlessness, self-sacrifice).',
  'n8n',
  'global',
  jsonb_build_object(
...
WITH existing AS (
  SELECT 1 FROM public.tools WHERE scope = 'global' AND slug = 'astrological_librarian'
)
INSERT INTO public.tools (name, slug, description, type, scope, config, enabled)
SELECT
  'Astrological Librarian',
  'astrological_librarian',
  'AstroCosmos Astrological Librarian via webhook (verification, correlation to Matrix).',
  'n8n',
  'global',
  jsonb_build_object(
