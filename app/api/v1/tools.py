...
from app.models.tools import ToolCreate, ToolUpdate, ToolOut, ToolAssignmentRequest
...
def get_tools_service(client_service: ClientService = Depends(lambda: ClientService())) -> ToolsService:
    # Note: actual DI should pass configured ClientService; this placeholder aligns with existing pattern
...
async def create_tool(payload: ToolCreate, svc: ToolsService = Depends(get_tools_service)):
    return await svc.create_tool(payload)


@router.patch("/tools/{tool_id}", response_model=ToolOut)
async def update_tool(tool_id: str, payload: ToolUpdate, client_id: Optional[str] = Query(None), svc: ToolsService = Depends(get_tools_service)):
    return await svc.update_tool(client_id, tool_id, payload)


@router.delete("/tools/{tool_id}")
async def delete_tool(tool_id: str, client_id: Optional[str] = Query(None), svc: ToolsService = Depends(get_tools_service)):
    await svc.delete_tool(client_id, tool_id)
    return {"success": True}


@router.get("/agents/{agent_id}/tools", response_model=List[ToolOut])
async def list_agent_tools(agent_id: str, client_id: str = Query(...), svc: ToolsService = Depends(get_tools_service)):
    return await svc.list_agent_tools(client_id, agent_id)


@router.post("/agents/{agent_id}/tools")
async def set_agent_tools(agent_id: str, payload: ToolAssignmentRequest, client_id: str = Query(...), svc: ToolsService = Depends(get_tools_service)):
