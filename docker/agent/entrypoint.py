...
"""
    ...
    # the root cause is incorrect tool instructions in the database. Fix the instructions,
    # don't add workarounds here.
    # Root cause fix: Updated Content Catalyst system_prompt_instructions to use native
    # function calling instead of JSON text output (2026-03-04).
...
async def agent_job_handler(ctx: JobContext):
    ...
    try:
        ...
        try:
            ...
            try:
                # DEBUG: Log metadata keys to diagnose tool transmission issue
                logger.info(f"🔍 [TOOL-DEBUG] Metadata keys available: {list(metadata.keys())}")
                logger.info(f"🔍 [TOOL-DEBUG] metadata.get('tools') = {metadata.get('tools')}")
                logger.info(f"🔍 [TOOL-DEBUG] metadata.get('tools_config') exists = {bool(metadata.get('tools_config'))}")
