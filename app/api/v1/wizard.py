...
async def list_abilities(
    ...
    abilities = [
        ...
        AbilityOption(
            ...
            description="AI-powered video editing via Descript. Remove filler words, silences, enhance audio, generate captions, and create highlight clips.",
...
async def get_voice_sample(
    ...
    # Get Platform Cartesia API key: try environment first, then platform database
    cartesia_api_key = os.getenv("CARTESIA_API_KEY")
    if not cartesia_api_key:
        try:
            admin_client = supabase_manager.admin_client
            # Try platform_api_keys table first
            try:
                pk_result = admin_client.table("platform_api_keys").select("key_value").eq("key_name", "cartesia_api_key").eq("is_active", True).maybe_single().execute()
                if pk_result.data:
                    cartesia_api_key = pk_result.data.get("key_value")
            except Exception:
                pass
            # Fall back to the first client that has a Cartesia key
            if not cartesia_api_key:
                ck_result = admin_client.table("clients").select("cartesia_api_key").neq("cartesia_api_key", "").limit(1).execute()
                if ck_result.data and ck_result.data[0].get("cartesia_api_key"):
                    cartesia_api_key = ck_result.data[0]["cartesia_api_key"]
        except Exception as db_err:
            logger.warning(f"Failed to load Cartesia key from database: {db_err}")
    if not cartesia_api_key:
        raise HTTPException(
            status_code=503,
