"""
Transcript Store Module for FastAPI

Handles storage of conversation transcripts with citations support.
Provides a unified interface for storing text conversation turns.
Mirror of docker/agent/transcript_store.py for the FastAPI layer.
"""
import logging
import uuid
...
async def store_turn(
    ...
    try:
        ...
        if conversation_id:
            try:
                ...
                if not existing.data:
                    ...
                    try:
                        supabase_client.table("conversations").insert(conversation_row).execute()
        ...
        # Store both rows (FastAPI uses sync Supabase client)
    ...
    try:
        ...
        if user_row_id and len(user_text) >= 8 and user_text.lower() not in trivial_messages:
            try:
                ...
                supabase_client.table("conversation_transcripts") \
