-- Mitra Politi Database Schema Setup (FIXED VERSION)
-- Corrected to avoid ivfflat dimension limit error
...
-- DROP TABLE IF EXISTS conversation_transcripts CASCADE;
...
CREATE TABLE IF NOT EXISTS conversation_transcripts (
    ...
    metadata JSONB,
    channel TEXT,
    embeddings vector(1024),  -- 1024 dimensions for compatibility
    agent_id UUID,
    content TEXT,
    message TEXT,
    role TEXT,
...
-- IMPORTANT: We're using only 1024-dimensional embeddings to avoid ivfflat issues
CREATE TABLE IF NOT EXISTS documents (
    id BIGSERIAL PRIMARY KEY,
    content TEXT,
    -- embedding vector(4096),  -- REMOVED: Exceeds ivfflat limit, use embeddings instead
    summary TEXT,
    metadata JSONB,
    ...
    embeddings vector(1024),  -- Primary embedding column
    embedding vector(1024),    -- Alias for compatibility (same dimensions)
...
CREATE TABLE IF NOT EXISTS document_chunks (
    ...
    chunk_index INTEGER,
    content TEXT,
    embeddings vector(1024),  -- 1024 dimensions
...
-- Create indexes for performance (non-vector indexes first)
...
-- Create vector indexes using ivfflat (all are 1024 dimensions now)
...
BEGIN
    ...
    RAISE NOTICE 'Note: All vector embeddings are using 1024 dimensions to avoid ivfflat limitations.';
END $$;